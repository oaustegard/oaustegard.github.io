...
export async function processQuotes(postInfo, sortOrder = 'popular') {
    debugLog.clear();
    ...
    try {
        ...
        if (sortOrder === 'popular') {
            quotePosts.sort((a, b) => {
                const engagementA = (a.likes || 0) + (a.reposts || 0);
                const engagementB = (b.likes || 0) + (b.reposts || 0);
                return engagementB - engagementA; /* Descending order */
            });
        } else if (sortOrder === 'chronological') {
            /* Already sorted by API, but ensure chronological order */
            quotePosts.sort((a, b) => a.id - b.id);
        }
